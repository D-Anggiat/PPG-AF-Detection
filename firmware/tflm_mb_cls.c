...
uint8_t g_mode = MODE_SENSOR; //Default mode sensor
//uint8_t g_mode = MODE_GOLDEN; //Default mode load data
uint8_t g_patient_idx = 0;
uint8_t g_window_idx = 0;
uint8_t g_golden_active = 0;
...
// #endif
        ...
        if(g_use_case == 0) {
                xprintf("AF Detection Inference (no camera)\n");
                ...
                if (max30102_init() == 0) {
                	...
                	//cv_mb_cls_run(&af_result);
#endif

			ppg_pipeline_init();
 	        	xprintf("Reading MAX30102 samples\n");
			while (1) {
			    uart_check_and_process();

			    if (g_mode == MODE_GOLDEN) {
				hx_drv_timer_cm55m_sec_delay_ms(10, TIMER_STATE_DC);
			    } else {
			        uint8_t available = 0;
			        max30102_available_samples(&available);
			        if (available > 0) {
			            for (int i = 0; i < available; i++) {
			                uint32_t red_value = 0;
			                if (max30102_read_hr_sample(&red_value) == 0) {
			                    ppg_pipeline_process_sample((float)red_value);
			                }
			            }
        		        }
    			    }
        		...
        		xprintf("MAX30102 init GAGAL, cek koneksi sensor.\n");
    		}
	}
